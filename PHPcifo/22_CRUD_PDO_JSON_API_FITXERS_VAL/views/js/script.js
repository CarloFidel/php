...
const inicializarEventos = () => {
  ...
  document.querySelector("#formUsuario").addEventListener("submit", (e) => {//Si hace submit es porque e ha enviado el formulario, ha devuelto un true
    e.preventDefault();
      insertarDato();
...
const mostrarDatos = (datos) => {
  ...
  datos.forEach((dato) => {
    ...
    fila.innerHTML = `
            ...
            <td class="tipo-usuario editable" data-tipo"${dato.tipo_usuario}">${dato.tipo_usuario === 1 ? "Editor" : "Registrado"
      }</td>
            <td>
...
const validarLimiteEditores = (nuevoTipo) => {
  ...
  if (nuevoTipo === "1" && editoresActuales >= 3) {
    mostrarMensaje("No se pueden tener mas de 3 ediores. Convierta a algun editor a tipo Registrado primero", true);
    return true;
  };
  return false;
}
...
const activarEdicion = (fila) => {
  ...
  <select class="edit-input">
    <option value="2" ${tipoUsuarioActual == "2" ? "selected" : ""
    }>Registrado</option>
    <option value="1" ${tipoUsuarioActual == "1" ? "selected" : ""
    }>Editor</option>
  </select>`;
